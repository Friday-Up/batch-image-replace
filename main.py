...
def load_input(excel_path: str, sku_col: str, image_dir: str):
...
def process_sku(page, sku: str, image_path: str):
...
def run_batch(excel_path: str, sku_col: str, image_dir: str, log_fn=print,
              wait_for_login_fn=None):
    """执行批量换图"""
    df, records = load_input(excel_path, sku_col, image_dir)

    no_image_count = sum(1 for _, row in df.iterrows() if row.get("备注") == "未找到匹配图片")
    if no_image_count:
    ...
    for i, record in enumerate(records, 1):
        idx = record["row_idx"]
        sku = record["sku"]
        log_fn(f"[{i}/{len(records)}] SKU: {sku} | 图片: {Path(record['image_path']).name}")
        try:
            process_sku(page, sku, record["image_path"])
            success += 1
            df.at[idx, "备注"] = "换图成功"
            log_fn(f"  ✓ 换图成功")
    ...
    df.to_excel(excel_path, index=False)
    log_fn(f"\n备注已写回 Excel")
    log_fn(f"处理完成: 成功 {success}/{len(records)}")
    if failed:
        log_fn(f"失败的 SKU: {', '.join(failed)}")
    return {"status": "done", "success": success, "total": len(records), "failed": failed}
...
<script>
  ...
  async function startRun() {
    ...
    await pywebview.api.start_run(excel, skuCol, imageDir);
  }
...
class Api:
    ...
    def start_run(self, excel, sku_col, image_dir):
        def _task():
            try:
                def login_wait():
                ...
                result = run_batch(excel, sku_col, image_dir,
...
def run_gui():
    ...
    window = webview.create_window(
        ...
        width=780, height=620, resizable=False,
        js_api=api
    )
    webview.start()
...
def main_cli():
    ...
    run_batch(args.excel, args.sku_col, args.image_dir)
